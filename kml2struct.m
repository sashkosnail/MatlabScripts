...
for ii = 1:Nos
    ...
    % Find Altitude Field
%     bucket = regexp(objectStrings{ii},'<altitude.*?>.+?</altitude>','match');
    % Clip off flags
    altitudeStr = regexprep(bucket{1},'<altitude.*?>(\s+)*','');
    altitudeStr = regexprep(altitudeStr,'(\s+)*</altitude>','');
    % Split coordinate string by commas or white spaces, and convert string
    % to doubles
    altitude = str2double(altitudeStr);
    ...
    kmlStruct(ii).Altitude = altitude;
end